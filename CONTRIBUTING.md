...
The primary project is the experimental OpenTelemetry processor in `otelprocessor/`.
...
Coordination tests start a local Redis protocol test server. Set
`JEVMETRICS_TEST_REDIS_ADDR=localhost:6379` to also run against a real test Redis
server; CI provides Redis 7.4 for this check. See [cluster documentation](docs/CLUSTERING.md).
...
manifest and processor module compatible when updating dependencies. Dependabot
...
`otelprocessor` module carries a copy of the same license for module distribution.
